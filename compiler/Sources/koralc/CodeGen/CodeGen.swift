public class CodeGen {
    ...
    private func generateGlobalNode(_ node: TypedGlobalNode) {
        switch node {
        case let .globalVariable(identifier, value, _):
    ...
    private func nextTemp(_ type: Type) -> String {
        tempVarCounter += 1
    ...
    private func generateExpression(_ expr: TypedExpressionNode) {
        switch expr {
        case let .intLiteral(value, _):
            buffer += String(value)
        ...
        case let .block(statements, finalExpr, _):
            buffer += "({ \n"
            withIndent {
                for stmt in statements {
                    addIndent()
        ...
        case let .arithmeticOp(left, op, right, _):
            buffer += "("
            generateExpression(left)
            buffer += " \(arithmeticOpToC(op)) "
            generateExpression(right)
            buffer += ")"
            
        case let .comparisonOp(left, op, right, _):
            buffer += "("
            generateExpression(left)
            buffer += " \(comparisonOpToC(op)) "
            generateExpression(right)
            buffer += ")"
            
        case let .ifExpr(condition, thenBranch, elseBranch, _):
            buffer += "("
            generateExpression(condition)
            buffer += " ? "
            generateExpression(thenBranch)
            buffer += " : "
            generateExpression(elseBranch)
            buffer += ")"
            
        case let .functionCall(identifier, arguments, _):
            buffer += "\(identifier.name)("
            buffer += arguments.map { arg in
