...
extension CodeGen {
  ...
  ///
  /// For `*self`/`*mutable self` methods, returns `nil` — no wrapper is needed since the actual method
...
extension CodeGen {
  ...
  /// - If `*self` / `*mutable self`: uses the actual method C name directly (no wrapper needed)
  ///
