...
## 导入命名空间
我们可以在导出语句的 `{}` 中，使用 `"名称"` 语句来导入其它的命名空间、库、框架到某个命名空间中。  
...
## 赋值
和常规的编程语言一样，我们需要用 `标识符 = 值` 语句就可以将右边的数据赋值给左边的标识符。
...
## 常量
常量是语言在编译时确定的，不可更改的，只支持基础类型的一种特殊类型，使用`标识符 类型 : 值`定义，`类型`通常可以省略。  
...
```
一旦一个方法被声明为异步方法，编译器会自动给返回值套上 `[Task 类型]` 类型包裹，这个方法就可以被异步执行了。
...
## 注解声明
我们只需要使用 `<>` 包裹住标记内容即可。
如果注解项目有子属性，只需要使用 `<>` 包裹即可。如果需要注明指定属性，则如结构体的简化构建一样使用 `标识符 = 表达式` 赋值即可。
