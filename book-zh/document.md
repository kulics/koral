...
```
这样便会创建一个包含 `a,b,c` 三个条目 的 `(Str, Int)Dictionary` 类型字典。

如果你需要一个明确类型的字典，同样可以使用构造函数来创建。

字典类型的表示方法是 `(键类型, 值类型)Dictionary`。

例如：
```
dictionary = (Int, Int)Dictionary{}     ` 空 `
```
### 访问
...
## 连续单条件判断
如果我们需要对一个表达式进行多个连续相等判断，可以使用`表达式 == | 表达式 ? {}`语句。
...
```
这种判断非常适合对某一个表达式的连续判断，避免编写过多的判断条件。
...
这里的 `me` 用来声明结构体自身，这样可以方便地访问自身的属性。它只是一个参数，因此可以自由使用 `me` 以外的标识符。
