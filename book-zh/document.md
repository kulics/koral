...
需要注意的是，如果我们的数据类型希望提供可变成员变量，那数据类型本身也需要被声明为 mut。这能帮助我们更容易区分可变类型和不可变类型。

成员变量的可变性是跟随类型的，与实例变量是否可变没有关系，所以我们即使声明了不可变变量也可以修改可变成员变量。

```feel
type mut Point(mut x: Int, mut y: Int);

let main() = {
    let a = Point(64, 128); ## `a` 不需要声明为 mut
    a.x = 2; ## ok
    a.y = 0 ## ok
}
```
