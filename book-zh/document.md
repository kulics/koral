...
特别的，由于 Lite 没有关键字，因此标识符的规则相比其它语言更加宽松，甚至可以支持空格。
...
例如:
```
(n, c) := Top sell()    # 将返回的两个值定义给 n 和 c #
(n, c) = Top sell()     # 将返回的两个值覆盖赋值给 n 和 c #
```
...
Each 1 to 10(Print)
```
如此，我们就在 `Each 1 to 10` 内部的循环中执行了 `print`函数。
...
Take( {a, b -> a + b} )
Find all{ it -> it > 7 }  # 如果函数只有一个参数，还可以省略括号 #
```
...
例如：
```
(me: Student) get girl friend() -> (name: Str) {
    <- (me._girl friend)
}
```
...
# 重写 #
(me: Chinese Student)(parent) get girl friend() -> (name: Str) {
    <- (parent._girl friend)
}
```
### 构造
...
(me: Student) <>(name: Str, number: Str) {
    ...
    me.number = number
    # 计算得出班级 #
    me.class = get sub text(number, 2, 3)
    # 计算得出年级 #
    me.grade = get sub text(number, 0, 1)
}
```
