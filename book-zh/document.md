...
在 Feel 内，语句是最小的组成单位，每个语句必须使用分号作为结束符。除此之外， Feel 对语句无任何其他限制，开发人员可以很随意地用符合自己风格的方式编写语句。  
...
由于 Feel 使用分号作为语句的结束符，所以当一条语句中的元素过长时，我们可以将一条语句分成多行编写。
...
Feel 的变量是一种绑定语义，相当于是把一个变量名和一个值绑定在一起，从而建立起了关联关系，类似于键值对。为了安全性的考虑，Feel 的变量默认是不可以改变的，当然Feel 也提供了另一种变量——可变变量。
...
一旦不可变变量被声明之后，它的类型和值在当前作用域内就不会再被改变。

如果我们尝试对不可变变量赋值，编译器会报错。
...
那么怎么使用这些定义好的函数呢？我们只需要使用 在函数表达式后面使用 `()` 语法就可以调用函数，从而得到函数的返回值。
...
结构是由一系列具有相同类型或不同类型的数据构成的数据集合，它是一种复合数据类型。

显而易见，结构适合用来将不同数据包装到一起，形成一个新类型，便于操作复杂的数据。
