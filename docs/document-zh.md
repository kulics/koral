...
1. `using module { symbol-list }` 只导入该模块公开的 public 符号。
2. `as` 作用于单个导入符号，而不是整个模块。
3. `using module { .. }` 导入该模块全部 public 符号，且 `..` 必须单独出现。
4. 导入名只在当前文件内可见，不会自动再导出。
5. 模块合法性由 manifest 中的 `deps` 决定，编译器不会再从目录结构里推断模块树。

已移除的源码形式：

- `using "file" as Name`
- `using Super...`
- `using Std.Io` 这类裸模块导入
- `using Std.Io.Reader` 这类成员导入
- `using Std.Io.*` 这类批量导入
- `using Std.Io as Io` 这类模块别名导入
