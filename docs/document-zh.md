...
- 语句可以显式使用分号 `;` 结束。
- 默认情况下，换行会结束当前语句。
- 行首续行的 ASI 白名单仍然只包含少数明确的接续 token：`and`、`or`、`is`、`then`、`else`、`.`、`->`。
- 因此像 `and then`、`or else`、`or return` 这类组合之所以能跨行成立，本质上仍然是因为它们分别以 `and` 或 `or` 开头，并在后续语法里继续归属于同一个结构。
- 另外，如果上一行以一个仍然需要右操作数的中缀运算符结尾，换行后也可以继续归属于同一个表达式。这包括算术、比较、逻辑/位运算以及 range 运算符，例如 `+`、`*`、`==`、`and`、`<<`、`..`。
...
- 空行和注释本身不会改变这个判断；是否续行仍然只取决于下一 token、上一行是否以尾随中缀运算符结束，以及当前是否还在未闭合分组里。
- 普通运算符仍然不允许以“新一行的行首”方式续接。也就是说，`+`、`*`、`==`、`<<`、`..` 这些 token 不能单独出现在续行行首，除非 parser 还处于未闭合分组内部。
...
// 不允许：普通运算符在新行行首仍然不会继续拼接。
let bad = 1
+ 2
```

上面的 `1 +` 会因为上一行以仍需右操作数的中缀运算符结尾，而继续归属于同一个表达式；但最后那个 `+ 2` 仍会被当成新语句开头，因此会触发语法错误。相对地，`(` ... `)` 分组中的 `+` 仍然合法，因为 parser 还没有离开未闭合分组。
