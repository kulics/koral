...
- `&mut` 仍然要求可写左值。
- `&raw` / `&raw mut` 要求可取地址的存储，因此字面量和临时值会被拒绝。
...
// let raw_bad = &raw 42  // 错误：raw 取址需要可取地址存储
...
- **引用（Reference）**：`*` / `*mut` 是托管引用。普通 `&` 可从左值或右值形成托管引用；`&mut` 仍要求可写左值；`&raw` / `&raw mut` 只会从可取地址存储形成 raw 指针。编译器通过逃逸分析决定托管引用走栈还是走堆，`box(expr)` 是显式托管构造。函数参数不允许隐式 ref 提升——调用方必须用 `&` 显式传入。普通参数不允许 auto-deref——传入 `*T` 给期望 `T` 的参数需要显式用 `*expr`。隐式转换仅对 method receiver（`self`）生效；其中 `*self` 可在右值接收者上通过物化临时值调用，而 `*mut self` 仍要求可写左值。Koral 使用所有权分析和逃逸分析来决定栈安全借用还是堆支持的引用计数，防止悬垂指针和内存泄漏。
...
2. 前缀: 一元 `-`, `~`，解引用 `*`，以及取址 `&`、`&mut`、`&raw`、`&raw mut`
3. 乘除: `*`, `/`, `%`
4. 加减: `+`, `-`
5. 移位: `<<`, `>>`
6. 比较: `==`, `<>`, `<`, `>`, `<=`, `>=`
