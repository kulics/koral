...
```koral
let mut n = 42
let a = ref n            // 从可变左值形成受管理引用
let b = deref a          // 解引用，得到 42
println(ref_count(a)) // 引用计数
```

`ref` 表示一种受语言管理的引用类型。`ref x` 会从可变左值形成这种引用。编译器会优先将它保持为栈安全的借用形式；当引用逃逸出作用域时，才会提升为基于堆的引用计数对象。像 `box(expr)` 这样的库函数则会有意构造同一种 `T ref`，只是它的形成方式是显式地产生逃逸引用。

注意：`T ref` 支持解引用、成员访问和方法分派，但不支持通过 `deref` 对整个目标值回写。解引用赋值（`deref x = v`）仅允许用于指针类型（`T ptr`）。
...
当期望类型已知时（例如变量声明有类型标注、函数参数有类型签名），可以省略类型名，直接使用 `.成员名` 语法构造枚举类型值或调用静态方法：
...
Trait object（`TraitName ref`）不支持直接 `deref`，应通过动态派发调用 trait 方法。
