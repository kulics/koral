...
Koral 支持基于 Trait 的运算符重载，覆盖算术、比较和下标访问这几类操作。
...
- `value[key]` -> `Index[K, V]`，对应 `ref_at(self ref, key K) ref V`
- `value[key] = expr` -> `MutIndex[K, V]`，对应 `mut_ref_at(self mut ref, key K) mut ref V`
...
自定义下标访问与下标赋值使用 `Index[K, V]` 和 `MutIndex[K, V]`：
...
- `Index[K, V]` / `MutIndex[K, V]`：下标访问与下标赋值。
- `ToString`：字符串转换。
...
Koral 中的**模块**由入口文件及其通过 `using` 声明依赖的所有模块组成。

- **根模块**：由编译入口文件及其依赖组成的模块
- **子模块**：可以是同级文件模块（`<name>.koral`）或子目录模块（`<name>/<name>.koral`）
...
- 后续字符只能使用小写字母、数字或 `_`。
- 在源码中，模块路径段使用 PascalCase 符号（例如 `my_tools.koral` 在代码中写作 `MyTools`）。
...
注意：文件合并语法（`using "file"`）不支持访问修饰符。

合并目标既可以是：
- 同级文件（如 `utils.koral`），也可以是
- 子目录模块入口（如 `utils/utils.koral`）。

文件查找相对于当前文件所在目录。字符串内容就是文件名（不含 `.koral` 后缀），不做任何大小写转换。

合并后的文件共享同一模块作用域，它们的 `public` 和 `protected` 符号互相可见。

#### 子模块声明

使用 `using "file" as Name` 声明子模块：

```koral
using "models" as Models                // 声明 Models 子模块（私有）
protected using "models" as Models      // 声明并在当前模块内共享
public using "models" as Models         // 声明并对外部模块公开
```
...
声明子模块后，可以从中导入特定成员或批量导入：

```koral
using Models.User      // 导入特定成员
using Models.*         // 批量导入所有 public 成员
```

#### 父模块访问

使用 `Super` 链访问同一编译单元内的父模块：
...
```koral
using Std                   // 导入 Std 模块
using Std.List              // 导入 Std.List 模块
using Std.Io as Io          // 使用模块别名导入
```

说明：

- 在 std 子模块中，根模块 `Std` 导出的 `public` 符号默认可见，无需重复 `using Std.X`。
- 别名语法统一为 `using module.path as alias`。
- 别名首字母大小写必须与被引用标识符首字母一致：
