...
```koral
using Self.Utils...      // 将同级模块 Utils 合并到当前模块
using Self.Helpers...    // 将同级模块 Helpers 合并到当前模块
```

注意：模块合并语法（`using path...`）不支持访问修饰符。

合并目标既可以是：
- 同级文件模块（如 `utils.koral`），也可以是
- 子目录模块入口（如 `utils/utils.koral`）。

合并后的模块共享同一作用域，它们的 `public` 和 `protected` 符号互相可见。
...
```koral
using Self.Models              // 导入 Models 子模块（私有）
...
```koral
using Self.Models.User      // 导入特定成员
using Self.Models.*         // 批量导入所有 public 成员
```
