...
given TcpSocket as Reader {
    public read(*self, into: *mutable List[UInt8], span: Range[UInt]) Result[UInt];
};

given TcpSocket as Writer {
    public write(*self, from: List[UInt8], span: Range[UInt]) Result[UInt];
...
given UdpSocket {
    ...
    public recv_from(*self, into: *mutable List[UInt8], span: Range[UInt]) Result[Pair[UInt, SocketAddr]];
    public connect[T IntoSocketAddr](*self, addr T) Result[Void];
    public send(*self, from: List[UInt8], span: Range[UInt]) Result[UInt];
    public recv(*self, into: *mutable List[UInt8], span: Range[UInt]) Result[UInt];
    public local_addr(*self) Result[SocketAddr];
    public peer_addr(*self) Result[SocketAddr];
